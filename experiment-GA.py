...
from GA.Wrappers.Gym import Gym
...
for repetiton_id in range(REPETITIONS):
    ...
    for generation in tqdm(range(GENERATIONS)):
        ...
        ## Train
        history = ga.train(surv=SURV, 
                    rand_surv=RAND_SURV,
                    noise=NOISE)

        last_avg = history['avg'][-1]
        last_median = np.median(list(ga.fitness.values()))
        ...
        if max(history['max']) == history['max'][-1]:
            ...
            names = list(ga.fitness.keys())
            f = list(ga.fitness.values())

            best = ga.gen[names[f.index(max(f))]]
            ga.save_specimen(best, DB_DIR+'/'+str(ID)+'.h5')
